...
class ApiService {
  ...
  // ... (login, logout, fetchOfficineInfo ... restent inchangés) ...
  ...
  Future<List<ProductSearchResult>> searchProducts(String query) async { try { final response = await _dio.get( '/vente/search', queryParameters: {'query': query, 'page': 1, 'start': 0, 'limit': 10}, ); if (response.statusCode == 200 && response.data['data'] is List) { return (response.data['data'] as List) .map((item) => ProductSearchResult.fromJson(item)) .toList(); } return []; } catch (e) { print("Error searching products: $e"); return []; } }
  ...
  Future<Map<String, dynamic>> cloturerVente({
    ...
    try {
      final response = await _dio.post('/vente/cloturer/vno', data: {
        ...
        // MODIFICATION : Utilisation des nouveaux champs
    ...
    try {
      final response = await _dio.post(
        ...
        data: {
          ...
          "reglements": [
            {
              // MODIFICATION : Le montant du règlement est le Net
              "montant": summary.montantNet,
              "montantAttentu": summary.montantNet,
              "typeReglement": typeReglementId
            }
